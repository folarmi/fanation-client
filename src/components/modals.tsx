...
function EditProfileModal() {
  const { profile, updateProfile, closeModal } = useAppStore();
  ...
  return (
    <>
      ...
      <div className="muted t13" style={{ marginBottom: 16 }}>
        Your name, handle and photos, as fans see them.
      </div>
      ...
      <input
        ...
        accept="image/*"
        hidden
        onChange={(e) => pickImage(e, setCoverUrl)}
      />
      <input
        ref={avatarInput}
        type="file"
        accept="image/*"
        hidden
        onChange={(e) => pickImage(e, setAvatarUrl)}
      />
      ...
      <div style={{ position: "relative", marginBottom: 44 }}>
        ...
        >
          ...
          >
            <Icon n="camera" s={12} c="#fff" />
          </div>
        </div>
      </div>

      <label className="label" htmlFor="edit-profile-name">
        Name
      </label>
      <input
        ...
        style={{ marginBottom: nameErr ? 6 : 14 }}
      />
      {nameErr && (
        <div className="coral t12" style={{ marginBottom: 14 }}>
          {nameErr}
        </div>
      )}

      <label className="label" htmlFor="edit-profile-handle">
        Username
      </label>
      <div
        className="row hair"
        style={{
          padding: "0 14px",
          borderRadius: 14,
          marginBottom: handleErr ? 6 : 14,
          borderColor: handleErr ? "rgba(243,106,70,.5)" : "var(--line)",
        }}
      >
      ...
      <div className="row gap10 wfull" style={{ marginTop: 4 }}>
        <button className="btn btn-ghost grow" onClick={closeModal}>
          Cancel
        </button>
        <button
          className="btn btn-blue grow"
          disabled={!!nameErr || !!handleErr}
          onClick={save}
        >
          Save
        </button>
